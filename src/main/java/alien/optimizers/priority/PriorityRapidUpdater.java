...
public class PriorityRapidUpdater extends Optimizer {
    ...
    public void updatePriority() {
        try (DBFunctions db = TaskQueueUtils.getQueueDB()) {
            ...
            StringBuilder updateQuery = new StringBuilder("INSERT INTO PRIORITY (userId, waiting, running, totalRunningTimeLast24h, totalCpuCostLast24h) VALUES ");
            ...
            try (Timing t = new Timing(monitor, "TQ_updatePriority_ms")) {
                ...
                if (!PriorityRegister.JobCounter.getRegistry().isEmpty()) {
                    ...
                    logger.log(Level.INFO, "Updating priority for active users: " + registrySize);
                    registerLog.append("Updating PRIORITY table for active users: ")
                            .append(registrySize)
                            .append("\n");
                    Timing t2 = new Timing(monitor, "TQ_updatePriority_db_ms");
                    t2.startTiming();
                    boolean query = dbdev.query(updateQuery.toString(), false);
                    ...
                    if (query) {
                        ...
                        t2.endTiming();
                        registerLog.append("Flushing values to database and subtracted " + registrySize + " counters completed successfully in " + t2.getMillis() + " \n");
                    }
