...
public class OldJobRemover extends Optimizer {
    ...
    private void startCron() {
        ...
        try (Timing t0 = new Timing(monitor, "OldJobRemover")) {
            ...
            t3.startTiming();
            boolean res = dbdev.query(deleteQuery);
            if (res) {
                int deleted = dbdev.getUpdateCount();
    ...
    private String getDeleteQuery(Set<Long> finalJobs) {
        ...
        for (Long id : finalJobs) {
            queueIds.append(id)
                    .append(",");
            TaskQueueUtils.putJobLog(id, "OldJobRemover", "Job to be removed by OldJobRemover optimizer", null);
        }
